import { date, decimal, integer, pgTable, serial, timestamp, varchar, primaryKey } from "drizzle-orm/pg-core";
...
export const expenseParticipants = pgTable("expense_participants", {
    /** 支出ID */
    expenseId: integer("expense_id").references(() => expenses.expenseId).notNull(),
    /** ラインユーザーID */
    ...
    primaryKey({columns: [table.expenseId, table.lineUserId]}),
]);