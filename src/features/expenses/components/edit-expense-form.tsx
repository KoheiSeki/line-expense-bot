...
type EditExpenseFormInnerProps = EditExpenseFormProps & {
	liff: typeof liff;
};

/** ローディング・LIFF 初期化待ちを担当するラッパー */
export const EditExpenseForm = ({ expense, members }: EditExpenseFormProps) => {
	...
	return <EditExpenseFormInner expense={expense} members={members} liff={liff} />;
};

const EditExpenseFormInner = ({
	expense,
	members,
	liff,
}: EditExpenseFormInnerProps) => {
	const {
		isPending,
		payerUserId,
		setPayerUserId,
		...
		updateShareAmount,
		handleSubmit,
	} = useEditExpenseForm({ expense, liff });
		...
		<form onSubmit={handleSubmit} className="flex flex-col gap-6 p-6">
			...
			<div className="bg-white rounded-2xl shadow-sm overflow-hidden divide-y divide-zinc-100">
				<div className="px-4 pt-3 pb-2">
					<Label htmlFor="edit-title" className={`${LABEL_CLASS} mb-1`}>
						タイトル
					</Label>
					<Input
						id="edit-title"
				...
				<div className="px-4 pt-3 pb-2">
					<Label htmlFor="edit-amount" className={`${LABEL_CLASS} mb-1`}>
						合計金額
					</Label>
				...
				<div className="px-4 pt-3 pb-2">
					<Label htmlFor="edit-paidAt" className={`${LABEL_CLASS} mb-1`}>
						支払い日
					</Label>
					<Input
						id="edit-paidAt"
			...
			<div className="bg-white rounded-2xl shadow-sm px-4 py-4">
				<Label htmlFor="edit-payerUserId" className={`${LABEL_CLASS} mb-3`}>
					支払い者
				</Label>
			...
			<div className="bg-white rounded-2xl shadow-sm overflow-hidden">
				<div className="px-4 pt-4 pb-2">
					<Label htmlFor="edit-participants" className={LABEL_CLASS}>
						参加者と負担金額
					</Label>
				</div>
